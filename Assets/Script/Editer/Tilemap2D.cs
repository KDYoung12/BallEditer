...
{
    [SerializeField]
    private GameObject tilePrefabs; // 맵에 배치되는 타일 프리펩
    ...
    // 맵이 배치되는 타일 정보 저장을 위한 List 프로퍼티
    ...
    public void GeneraterTilemap()
    {
        // out 또는 ref 키워드를 사용할 때 프로퍼티 사용이 불가능하기 때문에 지역 변수 선언
        int width, height;

        // InputField에 있는 width, height 문자열은 width, height 변수에 정수로 저장
    ...
    {
        // 맵에 배치된 모든 타일의 정보를 mapData.mapData 배열에 지정
